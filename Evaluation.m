...
%              figure,imshow(E)
     ...
     Out = Out(:,120:650,:);
    imwrite(mat2gray(Out),['out',num2str(Idx),'.tif'],'tif')
     [L1,L2] = maps2labels3( Img,AD(:,:,Idx));
     Out = image_result(Img,L1,L2);
     Out = Out(:,120:650,:);
    imwrite(mat2gray(Out),['out',num2str(Idx),'_AD.tif'],'tif')
    [L1,L2] = maps2labels3( Img,GT1(:,:,Idx));
     Out = image_result(Img,L1,L2);
     Out = Out(:,120:650,:);
    imwrite(mat2gray(Out),['out',num2str(Idx),'_GT1.tif'],'tif')
    
    [L1,L2] = maps2labels3( Img,GT2(:,:,Idx));
     Out = image_result(Img,L1,L2);
     Out = Out(:,120:650,:);
    imwrite(mat2gray(Out),['out',num2str(Idx),'_GT2.tif'],'tif')
    else
        break
    end
end
...
save('Benchmark','Error','Error1','Error2')